...
async function scrape(page: number = 1): Promise<IScrapeReturn> {
  ...
  const TRENDING =
    page > 1
      ? `https://forums.redflagdeals.com/hot-deals-f9/trending/${page}`
  ...
  deals = $("ul.topiclist.topics.trending.with_categories li.row.topic")
    .map((index, element) => {
      const title = $(element).find("h3.topictitle a.topic_title_link").text();
      const url = $(element)
        .find("h3.topictitle a.topic_title_link")
        .attr("href");
      ...
      const replies = $(element)
        .find(".thread-meta-small .thread_meta_small_replies")
        .text()
        .trim();
      const last_updated = $(element)
        .find(".thread-meta-large .last_updated_post_time")
        .text()
        .trim();
...
function sanitize(deal) {
  ...
  try {
    ...
    const date = moment(rawDate, "MMM D[st|nd|rd|th], YYYY h:mm A")
      .utc()
      .toISOString();
