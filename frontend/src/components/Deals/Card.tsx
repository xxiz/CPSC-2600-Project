    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      {deals.map((deal) => (
        <a
          ...
          href={deal.url}
        >
          <h3 className="mb-2 text-lg font-semibold">{deal.title.length > 50 ? deal.title.substring(0, 50) + `...` : deal.title}</h3>
          <p className="mb-2 text-gray-600">{deal.description}</p>
          <p className="mb-1 text-sm text-gray-500">{new Intl.DateTimeFormat('default', { month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true, timeZoneName: 'short' }).format(new Date(deal.last_updated))}</p>
